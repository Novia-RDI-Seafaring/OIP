...
**Target version:** 0.3 · **Status:** accepted · **Type:** backward-compatible additions
...
```json
{
  "id": "lkh-pump:p4-chart-series-b",
  "kind": "chart_series",
  "derived_from": "lkh-pump:p4-r1",
  "source_ref": {
    "kind": "pdf-page-bbox",
    "page": 4,
    "bbox": [56.5, 58.5, 252.8, 236.2],
    "page_size": [595.3, 841.9]
  },
  "content": { "data": { "x_label": "Q (m3/h)", "...": "..." } }
}
```
...
A region producer SHOULD expose a headless tool that takes the region's
content (or a reference to it) plus any extraction parameters, and either
writes the derived region(s) itself or returns the derived payload for
the consumer to file. The shipped reference does the latter:

```
graphtracer.trace_series(image_path, request) -> { axes, series }
```

and the consumer persists the result as a derived region (in anchor, via
its `derive_region` tool, which copies the parent's `source_ref` and
...
Add `data` as a content kind. Unlike the file-backed kinds, `data`
carries the structured JSON inline, as an object on the region. OIP does
not fix the shape of that object in general; the region `kind` implies
it, the same way a `transcript_segment` implies what its text means. OIP
defines one conventional shape per renderable token (see change C) so
portability is possible.

```json
"content": {
  "data": { "x_label": "Q (m3/h)", "y_label": "H (m)", "series": ["..."] }
}
```
...
A consumer that recognises `chart` and finds `data` content shaped
...
```
graphtracer.trace_series(image_path, request)
```

where `request` carries the calibration ticks and per-line seed
waypoints, all in pixels.
...
[`graph-data-extractor`](https://github.com/NoviaIntSysGroup/graph-data-extractor)
(a chart tracing tool) is the worked reference for this RFC. As of its
v0.1.0 it ships:

- a `consumes: { region_kinds: ["chart"], content_kinds: ["png"] }` block
  and a `produces: { region_kinds: ["chart_series"] }` block,
- a headless `graphtracer.trace_series(image_path, request)` tool that
  takes ticks and waypoints and returns `{ axes, series }`; the consumer
  files the result as a `chart_series` region whose `data` follows the
  `chart` contract and whose `source_ref` copies the parent chart region,
- a `ui_hints` entry mapping `graphtracer:chart_series` to
  `renders: "chart"`,
...
## 9. Worked `data` content

The `content.data` object of a `chart_series` region, following the
`chart` token's contract:
